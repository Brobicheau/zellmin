...
impl ZellijPlugin for State {
    ...
    fn render(&mut self, rows: usize, cols: usize) {
        ...
        match &self.status {
            ...
            Status::Ready => self.render_ready(rows, cols),
        }
    }
}

impl State {
    fn render_ready(&self, _rows: usize, _cols: usize) {
        let repo_root = self
            .repo_root
            .as_ref()
        ...
        println!("Worktree base: <repo>/{}/<branch>", self.config.worktree_dir_name);
        println!("Session name: <repo>-<branch>");
        println!();
        println!("> Branch: {}", self.branch_input);
        println!();
        ...
        match action {
            ACTION_DISCOVER_REPO => {
                if exit_code == Some(0) {
                    ...
                    if output.is_empty() {
                        self.status = Status::Error("Could not determine git repository root.".to_string());
                        return;
                    }
                    ...
                    self.status = Status::Error(command_error("Failed to discover repository root.", stderr));
                }
            }
            ACTION_CHECK_BRANCH => {
    ...
    fn worktree_path(&self, branch: &str) -> PathBuf {
        ...
        repo_root
            .join(&self.config.worktree_dir_name)
            .join(sanitize_path_segment(branch))
    }

    fn session_name(&self, branch: &str) -> String {
        let repo = self.repo_name.as_deref().unwrap_or("repo");
...
fn sanitize_session_segment(input: &str) -> String {
    let collapsed = input
        .chars()
        .map(|character| match character {
            'a'..='z' | 'A'..='Z' | '0'..='9' | '.' | '_' | '-' => character,
...
fn command_error(prefix: &str, stderr: Vec<u8>) -> String {
